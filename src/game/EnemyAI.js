...
export class EnemyAI {
  ...
  _updateDeathAnimation(delta) {
    ...
    if (this._deathStage === 0) {
      ...
      // v3.8 调整：阻尼从 0.95 提到 0.99（接近 1 = 几乎无衰减），冲量保持更久
      // 配合重力从 20 降到 5（重力矩极小，冲量主导整个倒地过程）
      // 目标：冲量匀速推动 AI 倒下，呈现"被击飞一直飞出去"的惯性效果，而非"先停顿再倒下"
      // 冲量 2.5-4 匀速旋转 MAX_TILT(1.551) 约需 0.39-0.62s，在目标 0.5-0.7s 范围内
      const impulseStrength = 2.5 + Math.random() * 1.5;
    ...
    // ---- 物理参数 ----
    // v3.8 调整：用户反馈"倒下的阻尼太大了"
    // v3.7 的 ANGULAR_DAMP=0.95 每秒衰减 95%，冲量在 0.256s 内从 1.537 衰减到 0.796（衰减 48%）
    // 导致速度曲线呈"U型"：冲量被消耗后重力才接管加速，视觉上"先停顿再倒下"，不像被击飞
    // v3.8 方案：阻尼提到 0.99（每秒仅衰减 45%），重力降到 5（重力矩极小），让冲量主导整个倒地过程
    // 预期：冲量匀速推动 AI 倒下，速度曲线接近水平线，呈现"被击飞一直飞出去"的惯性效果
    const GRAVITY = 5.0;               // 重力加速度（20 → 5，重力矩极小，冲量主导倒地）
    const ANGULAR_DAMP = 0.99;         // group 角速度阻尼（0.95 → 0.99，几乎无衰减，冲量保持）
    const JOINT_DAMP = 0.95;           // 关节角速度阻尼（0.90 → 0.95，关节惯性更明显）
